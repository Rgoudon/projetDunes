...
class MenuPrincipal(Frame):
	"""Classe pour la gestion de la fenetre principale. \n
	Permet de créer la fenetre principale du programme. \n
	La classe MenuPrincipal hérite de la classe tkinter Frame.
	...
	def __init__(self, fenetre):
		...
		**Paramètres d'instance :**
		...
		:param ResolutionAltitude: Correspont à la résolution de l'altitude de l'image (différence d'altitude entre deux niveaux de gris en cm).
		:type ResolutionAltitude: int 		
		...
		:param PrecedentChoixResolutionImage: Chaine de caractères permettant de sauvegarder le précédent choix dans la comboBox, pour éventuellement le remettre (si l'utilisateur annule son nouveau choix de résolution d'affichage de l'image).
		:type PrecedentChoixResolutionImage: string
		
		:param DessinPoint: Contient la liste des identifiants objet retournés par le constucteur tktinter lors de la contruction des points par l'utilisateur, permet de garder une référence sur l'objet en vue d'une suppresion.
		:type DessinLigne: liste d'entier 
						
		:param DessinLigne: Contient la liste des identifiants objet retournés par le constucteur tktinter lors de la contruction des axes par l'utilisateur, permet de garder une référence sur l'objet en vue d'une suppresion.
		:type DessinLigne: liste d'entier 

		:param CourantVersLaGauche: Donne le sens de lecture du courant de l'image.
		:type CourantVersLaGauche: Booleen
		
		:param SeuilDetectionPetiteDune: Objet de la classe spinbox permettant la selection de la taille des petites dunes.
		:type SeuilDetectionPetiteDune: Spinbox
		
		:param SeuilDetectionGrosseDune: Objet de la classe spinbox permettant la selection de la taille des grosses dunes.
		:type SeuilDetectionGrosseDune: Spinbox
		
		:param BoutonDupliquerAxe: Objet de la classe button permettant d'appeler la fonction de duplication d'un axe.
		:type BoutonDupliquerAxe: Button
						
		:param BoutonSupprimerDernierElement: Objet de la classe button permettant d'appeler la fonction de suppresion du dernier axe crée.
		:type BoutonSupprimerDernierElement: Button

		:param BoutonSupprimerAxes: Objet de la classe button permettant d'appeler la fonction de duplication d'un axe.
		:type BoutonSupprimerAxes: Button
		
		:param BoutonTraitementAxes: Objet de la classe button permettant d'appeler la fonction de traitemement des axes.
		:type BoutonTraitementAxes: Button
						
		:param BoutonTraitementImage: Objet de la classe button permettant d'appeler la fonction de traitement de l'image complete.
		:type BoutonTraitementImage: Button

		:param BoutonCoupure: Objet de la classe button permettant d'appeler la fonction de coupure sur l'image
		:type BoutonCoupure: Button
		"""
		...
		self.ResolutionAltitude = 0 
		self.ImageAffiche = 0
		self.PrecedentChoixResolutionImage = "" 
		self.DessinPoint = [] 
		self.DessinLigne = [] 
		self.CourantVersLaGauche = True 
		
		# Création d'un sous frame pour placer correctement les divers éléments sur la moitié gauche de la fenêtre
	...
	def ChargerImage(self):
		...
		"""
		# On ouvre une boite de dialogue ou l'utilisateur va choisir son image
...
Voulez-vous poursuivre ?""")
	...
	def ChoisirSensCourant(self):
		"""
		Setter pour l'attribut stockant le sens du courant.
		"""
	...
	def AjoutPointAffichage(self, PositionX, PositionY):
		"""
		Fonction qui ajoute la réference du point tracé dans la liste DessinPoint.                               
		"""
	...
	def AjoutLigneAffichage(self):
		"""
		Fonction qui ajoute la réference de la ligne tracé dans la liste DessinLigne.
		"""
	...
	def PlacementPoint(self, event):    
		"""
		Fonction d'ajout d'un point lors d'un clic souris sur l'image.
...
Impossible de créer un axe dont la longueur est nulle.""")
	...
	def DupliquerAxe(self):
		"""
		Fonction de duplication d'un axe.
		"""
		if(self.LesAxes.NombreAxes() < 1):
...
puis cliquer sur ce même bouton""")
	...
	def SupprimerDernierAxeOuPoint(self):
		"""
		Fonction de suppresion du dernier axe ou point placé.
		"""
	...
	def SupprimerAxes(self):
		"""
		Fonction de suppresion de tout les axes placés.
		"""
	...
	def DimensionMaxChoisi(self):
		"""
		Fonction de selection de la resolution de l'image.
		"""
	...
	def CoupureImage(self):
		"""
		Fonction d'affichage de la fenetre de sélection pour une coupure sur l'image. 
		"""
		fenTraitementImage = Toplevel()
